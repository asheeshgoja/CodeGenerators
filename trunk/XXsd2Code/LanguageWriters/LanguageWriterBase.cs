...
namespace XXsd2Code
{
    ...
    public abstract class LanguageWriterBase
    {
        ...
        protected void GenerateEnums(StreamWriter sw, Dictionary<String, List<EnumElement>> enumsToGenerateMap)
        {
            try
            {
                foreach (KeyValuePair<string, List<EnumElement>> kvp in enumsToGenerateMap)
                {
                    ...
                    foreach (EnumElement var in kvp.Value)
                    {
                        ...
                        //    entry = String.Format("{0}{1} = {2},", GetTab(), varName, var.Value);
                        //else
                        //    entry = String.Format("{0}{1} = {2}", GetTab(), varName, var.Value);
